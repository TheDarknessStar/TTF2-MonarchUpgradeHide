...
global bool MonarchUpgradeHide1 = false
global bool MonarchUpgradeHide2 = false
global bool MonarchUpgradeHide3 = false
global bool MonarchUpgradeHide_while = false
global bool MonarchUpgradeHide_TaskConstraint = false
...
void function MonarchUpgradeHide_mod() {
    if (!MonarchUpgradeHide_TaskConstraint) {
        print("[MonarchUpgradeHide][主程序已载入]")
		MonarchUpgradeHide_TaskConstraint = true
        MonarchUpgradeHide_while = true
        while (MonarchUpgradeHide_while) {
            if (MonarchUpgradeHide1 && MonarchUpgradeHide2 && MonarchUpgradeHide3) {
                print("[MonarchUpgradeHide][隐藏开始]")
                RuiSetString(file.cockpitRui, "titanInfo1", "")
                RuiSetString(file.cockpitRui, "titanInfo2", "")
                RuiSetString(file.cockpitRui, "titanInfo3", "")
                RuiSetString(file.cockpitRui, "titanInfo4", "")
                MonarchUpgradeHide_while = false
				MonarchUpgradeHide_TaskConstraint = false
				print("[MonarchUpgradeHide][隐藏完成]")
            }
            wait(1.0)
        }
    } else {
        print("[MonarchUpgradeHide][主程序未载入][禁止多开]")
    }
}
#if MP
